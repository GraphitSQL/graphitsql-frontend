...
export const verifyEmailAddress = async (
  ...
  try {
    ...
    >(API_ROUTES.auth.register(), payload, {
      headers: {
        'registration-token': window.localStorage.getItem(
          LocalStorageItem.TOKEN_FOR_REGISTRATION
        ),
      },
    });
    return data;
  ...
  async (): Promise<ResendVerificationCodeResponse> => {
    try {
      const { data } = await axiosInstance.get<
        string,
        AxiosResponse<ResendVerificationCodeResponse>
...
export const logoutRequest = async (): Promise<void> => {
  try {
    await axiosInstance.get<string, AxiosResponse<void>>(
...
export const refreshAccessToken = async (
  ...
  try {
    ...
    window.localStorage.setItem(
      LocalStorageItem.REFRESH_TOKEN,
      data.refreshToken
    );
    return data;
  } catch (e) {
