...
export const SidebarPanel: React.FC<SidebarPanelProps> = memo(
  ({ handleAddNode, nodes, updateNodeById, clearWorkSpace, removeNode }) => {
    ...
    const addTableField = useCallback((parent: TTableNode) => {
    ...
    const deleteTableField = useCallback((parent: TTableNode, fieldId: string) => {
      removeNode(fieldId);
      updateNodeById(parent.id, { childrenCount: parent.data.childrenCount - 1 });
    }, []);
